...
class MegaPlayProvider {
    ...
    anikotoBase = 'https://anikotoapi.site';
    megaPlayStreamBase = 'https://megaplay.buzz/stream';
    cache;
    constructor(cache) {
        this.cache = cache;
    }
    ...
    titleMatchesAnikoto(anime, query) {
        const q = this.normalizeTitle(query);
        if (!q)
            return false;
        ...
        return candidates.some((title) => title === q || title.startsWith(q) || title.includes(q) || q.includes(title));
    }
    async fetchAnikotoRecent(page, perPage = 100) {
        try {
            ...
            logger_1.default.warn({ error, page }, 'Anikoto recent-anime fetch failed');
            return null;
        }
    }
    async resolveAnikotoSeriesByMalId(malId) {
        ...
        for (let page = 1; page <= maxPages; page++) {
            ...
            if (!recent.pagination || page >= recent.pagination.total_pages)
                break;
        }
        this.cache.set(cacheKey, 0, 3600);
        return null;
    }
    async findAnikotoByTitle(query) {
    ...
    async fetchAnikotoSeries(seriesId) {
        if (!/^\d+$/.test(seriesId))
            return null;
        ...
        try {
            ...
            if (!payload.ok || !payload.data?.episodes?.length)
                return null;
            ...
            logger_1.default.warn({ error, seriesId }, 'Anikoto series fetch failed');
            return null;
        }
    }
    async resolveSeriesData(showId) {
    ...
    mapAnikotoToShow(anime) {
        return {
            ...
            type: anime.type,
            year: anime.year,
            episodeCount: typeof anime.episodes === 'number'
                ? anime.episodes
                : anime.episodes
    ...
    async search(options) {
        try {
            ...
            if (!data.data || data.data.length === 0)
                return [];
            const results = await Promise.all(data.data.slice(0, 8).map(async (anime) => {
            ...
            if (query && results.length > 0) {
                ...
                const bestIndex = results.findIndex((item) => item._id === bestMalId || item.name === best.title);
                if (bestIndex > 0) {
            ...
            logger_1.default.error({ error }, 'MegaPlay search failed');
            return [];
        }
    }
    ...
    async getStreamUrls(showId, episodeNumber, mode) {
        ...
        const series = await this.resolveSeriesData(showId);
        if (series?.episodes?.length) {
            const episode = series.episodes.find((item) => String(item.number) === targetEpisode) ||
                series.episodes[parseInt(targetEpisode, 10) - 1];
            if (episode) {
                const embedUrl = episode.embed_url?.[mode];
                const streamUrl = embedUrl ||
    ...
    async getShowMeta(showId) {
        try {
            const series = await this.resolveSeriesData(showId);
            if (series?.anime)
                return this.mapAnikotoToShow(series.anime);
            if (!/^\d+$/.test(showId))
                return null;
            const url = `${this.jikanBase}/anime/${showId}`;
            const response = await fetch(url, { headers: FETCH_HEADERS });
    ...
    async getShowDetails(showId) {
        try {
            if (!/^\d+$/.test(showId))
                return { status: '' };
            const series = await this.resolveSeriesData(showId);
            if (series?.anime?.status)
                return { status: series.anime.status };
