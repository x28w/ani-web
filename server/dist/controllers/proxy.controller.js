    ...
    handleEmbedProxy = async (req, res) => {
        ...
        try {
            ...
            let html = response.data
                .replace(/\b(src|href)=(["'])(\/\/[^"']+|\/(?!\/)[^"']*)\2/gi, (_match, attribute, quote, resourcePath) => {
                const assetUrl = resourcePath.startsWith('//')
                    ? `https:${resourcePath}`
                    : `${targetUrl.origin}${resourcePath}`;
                return `${attribute}=${quote}${assetUrl}${quote}`;
            })
                .replace(/url\((["']?)(\/\/[^"')]+|\/(?!\/)[^"')]+)\1\)/gi, (_match, quote, resourcePath) => {
                const assetUrl = resourcePath.startsWith('//')
                    ? `https:${resourcePath}`
                    : `${targetUrl.origin}${resourcePath}`;
                return `url(${quote}${assetUrl}${quote})`;
            });
            const iconPatchedHtml = html.replace(/(<script[^>]+\/app\/js\/vendor\/plyr\.min\.js[^>]*><\/script>)/i, `$1${iconProxyPatch}`);
            const patchedHtml = iconPatchedHtml.replace(/(<script[^>]+hls(?:\.min)?\.js[^>]*><\/script>)/i, `$1${playlistProxyPatch}`);
